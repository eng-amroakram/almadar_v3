...
class InDirectOfferService extends Controller
{
    public $name =  "العروض الغير مباشرة";
    ...
    public function __construct()
    {
        ...
        $this->name = auth()->user()->user_type == "office" ? "العروض" : "العروض الغير مباشرة";
    }
    ...
    public function contents($type)
    {
        ...
        $inputs = [
            ...
            [
                ...
                select("select", "directions",  "directions_select_id$prefix_id", "fas fa-street-view", "", "select inputSelect$type", "الاتجاه", false, $directions, "multiple", true, "الاتجاه", "text-danger directions-validation fw-bold ms-5 reset-validation"),
                select("select", "land_type",  "land_type_select_id$prefix_id", "fas fa-earth-oceania", "", "select inputSelect$type", "نوع الارض", false, $land_types, "", true, "نوع الارض", "text-danger land_type-validation fw-bold ms-5 reset-validation"),
                select("select", "licensed",  "licensed_select_id$prefix_id", "fas fa-id-badge", "", "select inputSelect$type", "الترخيص", false, $licensed, "", true, "الترخيص", "text-danger licensed-validation fw-bold ms-5 reset-validation"),
                select("select", "street_width",  "street_width_select_id$prefix_id", "fas fa-road", "", "select inputSelect$type", "عروض الشوارع", false, $street_widths, "multiple", true, "عروض الشوارع", "text-danger street_width-validation fw-bold ms-5 reset-validation"),
                input("text", "character", "character_input_id$prefix_id", "fas fa-c", "rtl", "255", "form-control inputText$type", "الحرف او المجاور", true, "الحرف او المجاور", "text-danger character-validation fw-bold ms-5 reset-validation"),
                input("number", "interface_length", "interface_length_input_id$prefix_id", "far fa-building", "ltr", "255", "form-control inputText$type", "Example: 30", true, "طول الواجهة", "text-danger interface_length-validation fw-bold ms-5 reset-validation"),
                ...
                select("select", "owner_ship_type",  "owner_ship_type_select_id$prefix_id", "far fa-building", "", "select inputSelect$type", "نوع الملكية", false, $owner_types, "", true, "نوع الملكية", "text-danger owner_ship_type-validation fw-bold ms-5 reset-validation"),
                select("select", "building_type",  "building_type_select_id$prefix_id", "far fa-building", "", "select inputSelect$type", "نوع البناء", false, $building_types, "", true, "نوع البناء", "text-danger building_type-validation fw-bold ms-5 reset-validation"),
                select("select", "building_status",  "building_status_select_id$prefix_id", "far fa-building", "", "select inputSelect$type", "حالة البناء", false, $building_status, "", true, "حالة البناء", "text-danger building_status-validation fw-bold ms-5 reset-validation"),
                select("select", "construction_delivery",  "construction_delivery_select_id$prefix_id", "far fa-building", "", "select inputSelect$type", "تسليم البناء", false, $construction_deliveries, "", true, "تسليم البناء", "text-danger construction_delivery-validation fw-bold ms-5 reset-validation"),
            ],
            [
                ...
                select("select", "brokers_ids", "brokers_ids_select_id$prefix_id", "fas fa-user-tag", "", "select inputSelect$type", "الوسيط", true, brokers(true), "multiple", true, "الوسيط", "text-danger brokers_ids-validation fw-bold ms-5 reset-validation"),
            ]
        ];
